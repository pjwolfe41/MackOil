                ...
                int fit_computed, double fit, double resid)
{
    ...
    for (i = 0; i < selects.nspec; i++)
    {
        double data = 0;
		double value; 

        switch (selects.condition[i].datatype)
		{
			case CODE:
				data = code;
				break;
...
#define  EPS .000001
		value = selects.condition[i].value;
		switch (selects.condition[i].logop)
		{
			case LT:
				if (data >= value)
					select_point = 0;
		...
		if (selects.condition[i].or_next)
		{
			if (select_point)
				++i;			/* skip OR clause */
			else
