...
/*	process arguments	*/
    ...
    if (argc < 3)
        usage (argv[0]);
    dataname = argv[1];
    if (stricmp (argv[2], "code") == 0)
        type = CODE;
    else if (stricmp (argv[2], "obs") == 0)
        type = OBS;
    else if (stricmp (argv[2], "fit") == 0)
        type = FIT;
    else if (stricmp (argv[2], "resid") == 0)
        type = RESID;
    else
        usage (argv[0]);
    *data_type = type;
...
/*	compute fit and residual	*/
	...
	f = (float *) malloc ((unsigned) npoint * sizeof (float));
	r = (float *) malloc ((unsigned) npoint * sizeof (float));
	if (f == NULL || r == NULL)
	    error_stop ("cannot allocate space for fit and residual", "");

	strcpy (filename, fitdataname);
	strcat (filename, ".fit");
